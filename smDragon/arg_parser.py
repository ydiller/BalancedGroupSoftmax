...
class ArgParser(object):
    ...
    def _build_parser(self, parser):
        ...
        parser.add_argument("--train_dist", type=str,
                            default="dragon",
                            help='Distribution function for train set - dragon or fewshot (for benchmarking)')
        ...
        # dragon evaluation related arguments
        parser.add_argument("--eval_dist", type=str,
                            default="dragon",
                            help='Distribution function for val and test - dragon or flat')
        parser.add_argument("--eval_max_samples", type=int,
                            default=10,
                            help='Max samples for class in val and test -'
                                 ' starting point for distribution function')
        parser.add_argument("--eval_n_repeat", type=int,
                            default=10,
                            help='Num of repeats to average score over val and test')
        parser.add_argument("--eval_seed", type=int,
                            default=42,
                            help='Seed for evaluating val and test average accuracies')
...
class VisualExpertParser(ArgParser):
    def __init__(self, model_name):
        ...
        print(self.args, self.unknown_args)
...
class GaterExpertParser(ArgParser):
    def __init__(self, model_name, *expert_names):
        ...
        # merge gater params to the combined args
    ...
    def _add_model_arguments(self, parser):
        parser.add_argument("--topk", type=int,
                            ...
                            help='K value when applying topK on experts outputs. -1 means take '
                                 'experts ouput without any change')
        parser.add_argument("--sort_preds", type=int,
                            default=0,  # False
                            help='Should experts outputs be sorted before they get to the gater?')
        parser.add_argument("--freeze_experts", type=int, default=0,  # False
                            help='Should freeze experts whene training gater')
        parser.add_argument("--nparams", type=int, default=4,
                            help='Params number to learn')
        return parser

class GaterCifarParser(ArgParser):
    def __init__(self, model_name):
        super().__init__(model_name)
        print(self.args, self.unknown_args)
...
def gater_args():
    global UserArgs
    UserArgs = GaterExpertParser("Gater", "Visual", "LAGO").args

def smgater_args():
    global UserArgs
    UserArgs = GaterExpertParser("Gater", "VisualLWS").args

def gater_cifar_args():
    global UserArgs
    UserArgs = GaterCifarParser("Gater").args